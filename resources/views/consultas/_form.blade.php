...
<div class="grid grid-cols-2 gap-4">
    <div>
        ...
        <select name="idPaciente" class="w-full p-2 border rounded">
    ...
    <div>
        ...
        <select name="idTipoConsulta" class="w-full p-2 border rounded">
    ...
    <div>
        ...
        <input type="date" name="fechaIngreso" value="{{ old('fechaIngreso', isset($consulta) && $consulta->fechaIngreso ? $consulta->fechaIngreso->format('Y-m-d') : '') }}" class="w-full p-2 border rounded">
        @error('fechaIngreso')<div class="text-red-500 text-sm">{{ $message }}</div>@enderror
    </div>

    <div>
        <label class="block">Hora (HH:MM:SS)</label>
        <input type="text" name="hora" value="{{ old('hora', isset($consulta) ? $consulta->hora : '') }}" class="w-full p-2 border rounded" placeholder="14:30:00">
        @error('hora')<div class="text-red-500 text-sm">{{ $message }}</div>@enderror
    </div>

    <div class="col-span-2">
        <label class="block">Motivo</label>
        <textarea name="motivo" class="w-full p-2 border rounded" rows="3">{{ old('motivo', $consulta->motivo ?? '') }}</textarea>
        @error('motivo')<div class="text-red-500 text-sm">{{ $message }}</div>@enderror
    </div>

    <div class="col-span-2">
        <label class="block">Observación</label>
        <textarea name="observacion" class="w-full p-2 border rounded" rows="4">{{ old('observacion', $consulta->observacion ?? '') }}</textarea>
